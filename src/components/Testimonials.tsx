        ...
        <div className={`relative transition-all duration-1000 ${isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
                  ...
                  <div className="relative z-10">
                    <div className="w-32 h-32 mx-auto mb-6 relative">
                      <img 
                        ...
                        className="w-full h-full rounded-full object-cover border-4 border-gold/30 shadow-2xl"
                      />
          ...
          <div className="flex items-center justify-between mt-8">
            <button
              ...
              className="w-12 h-12 bg-gold/20 hover:bg-gold/30 border border-gold/30 rounded-full flex items-center justify-center transition-all duration-300 hover:scale-110"
            >
        ...
        <div className={`mt-16 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{ transitionDelay: '500ms' }}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {testimonials.slice(0, 3).map((testimonial, index) => (
              <Card 
                key={testimonial.id}
                className={`bg-gradient-to-br from-gray-900/50 to-black/50 border-gold/10 hover:border-gold/30 transition-all duration-500 cursor-pointer group ${
                  index === currentTestimonial ? 'ring-2 ring-gold/30' : ''
                }`}
                onClick={() => goToTestimonial(index)}
              >
