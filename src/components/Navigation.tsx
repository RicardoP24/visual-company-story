...
export const Navigation = () => {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${
      scrolled 
        ? 'bg-black/95 backdrop-blur-md border-b border-gold/30 shadow-2xl' 
        : 'bg-black/90 backdrop-blur-sm border-b border-gold/20'
    }`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isOpen && (
          <div className="md:hidden transition-all duration-300">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-black/95 backdrop-blur-md rounded-b-lg border-b border-gold/20">
              {navItems.map((item) => (
                <a
                  key={item.name}
                  href={item.href}
